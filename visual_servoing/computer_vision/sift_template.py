...
def cd_template_matching(img, template):
	...
	for scale in np.linspace(1.5, .5, 50):
		...
		top_left, bottom_right = tuple(max_loc), (max_loc[0] + w, max_loc[1] + h)
		# print(max_val)
		# print(min_loc)
		# cv2.rectangle(img, top_left, bottom_right, (0, 0, 255), 2)
		# image_print(img)
		if best_match is None or best_match[1] < max_val:
			best_match = [(top_left, bottom_right), max_val]
		...
		########### YOUR CODE ENDS HERE ###########
	# cv2.rectangle(img_canny, best_match[0][0], best_match[0][1], 255, 2)
